...
class County extends AbstractModel
{
    ...
    {
        $this->init('i18n_county', 'id', ['id', 'parent_id', 'code', 'default_name']);
    }
    ...
    protected function afterSave()
    {
        ...
        if (isset($this->storage['name'])) {
            $tableGateway = $this->getTableGateway('i18n_county_name');
            foreach ((array) $this->storage['name'] as $languageId => $name) {
                $language = new Language();
                $language->load($languageId);
                $this->upsert(['name' => $name], ['county_id' => $this->getId(), 'locale' => $language['code']], $tableGateway);
            }
        }
        $this->commit();
    }
    ...
    {
        $select->join('i18n_county_name', 'i18n_county_name.county_id=i18n_county.id', ['name'], 'left');
        $select->join('core_language', 'i18n_county_name.locale=core_language.code', ['language_id' => 'id', 'language' => 'name'], 'left');
