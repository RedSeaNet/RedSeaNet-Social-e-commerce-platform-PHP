(function (factory) {
    ...
    $(function () {
        ...
        $("input[name=qty]").on("change.redseanet", function () {
            calculatePrice();
        });
        var calculatePrice = function () {
            let bargainPrice = $('input[name=price]').val();
            $('#preview [data-price]').text(formatPrice(bargainPrice));
            let shippingFee = 0;
            ...
            $('span.subtotal').text(formatPrice(parseFloat(bargainPrice) * parseInt($('input[name=qty]').val())));
            $('span.shipping_fee').text(shippingFee);
            let total = parseFloat(bargainPrice) * parseInt($('input[name=qty]').val()) + shippingFee;
